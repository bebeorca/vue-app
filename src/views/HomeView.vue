<template>
  <div id="default-carousel" class="relative w-full mt-8 border-b border-b-black py-2" data-carousel="slide">
    ...
    <div class="relative h-56 overflow-hidden md:h-96 " >
        <div class="hidden duration-700 ease-in-out" data-carousel-item v-for="(post, index) in this.newPost" :key="index">
    ...
    <button type="button" class="absolute top-0 start-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
        <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 outline outline-1 dark:group-focus:ring-gray-800/70 group-focus:outline-none">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    <div v-for="(post, index) in this.newPost" :key="index">
      <img class="h-72 border-b w-screen border-b-black pb-2 overflow-hidden object-cover" :src="getImageUrl(post.file_path)"  alt="">
    </div>
  </div>
  <nav aria-label="Page navigation example">
    <ul class="flex justify-center -space-x-px text-sm">
      ...
      <li>
        <a href="#" :current="page" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">1</a>
      </li>
      <li>
